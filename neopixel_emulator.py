...
class NeoPixel_Emulator(pyglet.window.Window):
    ...
    def draw_LEDs(self, led_number):
        for led in range(led_number):
            img = pyglet.image.load('./NeoPixel-Emulator/ws2812b.png')
            ...
            self.color_sprites.append(pyglet.sprite.Sprite(img=pyglet.image.load('./NeoPixel-Emulator/circle.png'),batch=self.batch,x=(led-50*(led//50))*35,y=self.height-34-((led//50)*34)-5,group=self.color_group))
    ...
    def draw_LED_matrix(self, width, height):
        for y in range(height):
            for x in range(width):
                ...
                self.color_sprites.append(pyglet.sprite.Sprite(img=pyglet.image.load('./NeoPixel-Emulator/circle.png'),batch=self.batch,x=x*35,y=self.height-34-(y*34)-5,group=self.color_group))
